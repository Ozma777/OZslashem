...
E void FDECL(savebones, (int, time_t, struct obj *));/* changed also to include realtime*/
...
E void FDECL(unset_seenv, (struct rm *, int, int, int, int));/* nethack 360*/
...
#endif
/*E long FDECL(yyyymmdd, (time_t)); out by nethack360*/
...
E boolean NDECL(towel_day);	/* KMH -- Douglas Adams' Birthday */
E boolean NDECL(creator_day);	/*My Birthday August 18th */
E int NDECL(night);
...
E int FDECL(trickbag, (struct obj *));
...
E void FDECL(place_lregion, (XCHAR_P,XCHAR_P,XCHAR_P,XCHAR_P,
			     ...
			     XCHAR_P,d_level *));
E void FDECL(fumaroles, (BOOLEAN_P));
...
E struct obj *FDECL(mkcorpstat_, (int,int,BOOLEAN_P));
...
#ifdef BARD
E int FDECL(pet_can_sing, (struct monst *));
for later... above commented out already (broken?) the below is in dNethack's extern.h
...
E int FDECL(put_into_container,(struct obj *,struct obj *));
#endif
...
E int NDECL(dodip);
E void FDECL(ferment, (genericptr_t, long));
...
#endif/* doesn't work
E void FDECL(drop_earth, (int,int,int,BOOLEAN_P,BOOLEAN_P));*/
...
E struct monst *FDECL(qname, (struct monst *));		/* see sounds.c */
E int NDECL(dotalk);
#ifdef USER_SOUNDS
...
E int FDECL(disarm_fire_trap, (struct trap *));  check beartraps and holepatch cause this BROKE*/
...
E long FDECL(stop_timer, (SHORT_P, genericptr_t));out per nethack 360*/
...
E boolean FDECL(obj_has_timer, (struct obj *, SHORT_P));/*added from nethack360 to*/
E void FDECL(spot_stop_timers, (XCHAR_P, XCHAR_P, SHORT_P));
E long FDECL(spot_time_expires, (XCHAR_P, XCHAR_P, SHORT_P));
E long FDECL(spot_time_left, (XCHAR_P, XCHAR_P, SHORT_P));/* here*/
