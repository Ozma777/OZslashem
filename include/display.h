...
#define display_self()							\
	...
	youmonst.m_ap_type == M_AP_OBJECT ?				\
				objnum_to_glyph(youmonst.mappearance) : \
	/* else M_AP_MONSTER *//* monnum_to_glyph(youmonst.mappearance)) took all this out and replaced with 360*/
...
/*#define GLYPH_INVISIBLE GLYPH_INVIS_OFF out as of nethack 360*/
#define NO_GLYPH          MAX_GLYPH
...
#define pet_to_glyph(mon) ((int) what_mon(monsndx((mon)->data))+GLYPH_PET_OFF) nethack360 out*/
#define warning_to_glyph(mwarnlev) ((mwarnlev) + GLYPH_WARNING_OFF)
...
/* This has the unfortunate side effect of needing a global variable	*/
/* to store a result. 'otg_temp' is defined and declared in decl.{ch}.	*//*took all this out as per nethack360
#define random_obj_to_glyph() 					    	      \
	((otg_temp = random_object()) == CORPSE ?			      \
	    random_monster() + GLYPH_BODY_OFF :				      \
	        otg_temp + GLYPH_OBJ_OFF)
...
/* nethack 360 (or 342)doesn't have this
...
#define body_to_glyph(mnum)	((int) (mnum) + GLYPH_BODY_OFF)/* this is off in 360*/
 ...
 * considered objects.
 *//* taking a chance and killing this WHOLE thing to replace with nethack360
