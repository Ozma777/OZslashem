...
int mndx;
{
	...
	case PM_DRAKELING_ZOMBIE:
	case PM_DRAKELING_MUMMY:    mndx = PM_DRAKELING;  break;
	...
	case PM_MIND_FLAYER_MUMMY:    mndx = PM_ILLITHID;  break;
	case PM_ETTIN_ZOMBIE:
	case PM_ETTIN_MUMMY:	mndx = PM_ETTIN;  break;
	case PM_NYMPH_ZOMBIE:
	case PM_NYMPH_MUMMY:    mndx = PM_NYMPH;  break;
	case PM_RATLING_ZOMBIE:
	case PM_RATLING_MUMMY:    mndx = PM_RATLING;  break;
	case PM_SYLPH_ZOMBIE:
	case PM_SYLPH_MUMMY:    mndx = PM_SYLPH;  break;
...
/* Quest guardians */
	...
	case PM_AIRHEAD:     mndx = mode ? PM_AETHERIC_MAGE  : PM_HUMAN; break;
...
#ifdef CONVICT
	case PM_INMATE:      mndx = mode ? PM_CONVICT   : PM_HUMAN; break;
#endif
	case PM_FELLOW_EXPLORER:      mndx = mode ? PM_EXPLORATOR   : PM_HUMAN; break;
	case PM_DIGGER:		mndx = mode ? PM_EARTH_MAGE    : PM_HUMAN; break;
	case PM_GLADIATOR:   mndx = mode ? PM_FIGHTER    : PM_HUMAN; break;
	case PM_IGNITER:   mndx = mode ? PM_FLAME_MAGE    : PM_HUMAN; break;
	case PM_HACKER:   mndx = mode ? PM_GEEK    : PM_HUMAN; break;
	case PM_ATTENDANT:   mndx = mode ? PM_HEALER    : PM_HUMAN; break;
	case PM_MUSICIAN:   mndx = mode ? PM_INSTRUMENTALIST    : PM_HUMAN; break;
	case PM_FROSTER:   mndx = mode ? PM_ICE_MAGE    : PM_HUMAN; break;
	case PM_PADAWAN:   mndx = mode ? PM_JEDI    : PM_HUMAN; break;
	case PM_PAGE:        mndx = mode ? PM_KNIGHT    : PM_HUMAN; break;
	case PM_ABBOT:       mndx = mode ? PM_MONK      : PM_HUMAN; break;
	case PM_METALIST:    mndx = mode ? PM_METAL_MAGE  : PM_HUMAN; break;
	case PM_EMBALMER:   mndx = mode ? PM_NECROMANCER    : PM_HUMAN; break;
	case PM_ACOLYTE:     mndx = mode ? PM_PRIEST    : PM_HUMAN; break;
	case PM_BUCCANEER:   mndx = mode ? PM_PIRATE    : PM_HUMAN; break;
	...
	case PM_RESEARCHER:   mndx = mode ? PM_SCIENTIST    : PM_HUMAN; break;
#ifdef TOURIST
	case PM_GUIDE:       mndx = mode ? PM_TOURIST   : PM_HUMAN; break;
#endif
	case PM_EXTERMINATOR:       mndx = mode ? PM_UNDEAD_SLAYER   : PM_HUMAN; break;
	case PM_APPRENTICE:  mndx = mode ? PM_WIZARD    : PM_HUMAN; break;
	case PM_WARRIOR:     mndx = mode ? PM_VALKYRIE  : PM_HUMAN; break;
	case PM_YEOMAN_WARDER:     mndx = mode ? PM_YEOMAN  : PM_HUMAN; break;
	default:
		if (mndx >= LOW_PM && mndx < NUMMONS) {
			...
			else if (is_orc(ptr))   mndx = PM_ORC;/* NOT IN MONDATA.H YET*/
			else if (is_kobold(ptr))   mndx = PM_KOBOLD;
			else if (is_hobbit(ptr))   mndx = PM_HOBBIT;
			else if (is_ogre(ptr))   mndx = PM_OGRE;
			else if (is_giant(ptr))   mndx = PM_GIANT;
			else if (is_troll(ptr))   mndx = PM_TROLL;
			else if (is_illithid(ptr))   mndx = PM_ILLITHID;
			else if (is_sylph(ptr))   mndx = PM_SYLPH;
			else if (is_ratling(ptr))   mndx = PM_RATLING;
			else if (is_drakeling(ptr))   mndx = PM_DRAKELING;
		}
		break;
	}
	return mndx;
...
register struct monst *mtmp;
{
	...
	switch(mndx) {
		...
		goto default_1;
		/* all bad food should get food as "corpse" */
		...
		newsym(x, y);
		case PM_KILLER_TOMATO:
		(void) mksobj_at(TOMATO, x, y, TRUE, FALSE);
		newsym(x, y);
		case PM_BLIND_MELON:
		(void) mksobj_at(MELON, x, y, TRUE, FALSE);
		newsym(x, y);
		case PM_VICIOUS_MUSHROOM:
		(void) mksobj_at(MUSHROOM, x, y, TRUE, FALSE);
		newsym(x, y);
		return (struct obj *)0;
	    case PM_VAMPIRE:
	    case PM_VAMPIRE_LORD:
		...
		num = rnd(4);
		while (num--)
			obj = mksobj_at(SHEAF_OF_HAY, x, y, TRUE, FALSE);
		mtmp->mnamelth = 0;
		break;
	    default_1:
	    default:
...
register struct monst *mtmp;
{
	...
	mtmp->mhp -= dam;
	if (mtmp->mhpmax > dam) mtmp->mhpmax -= (dam+1) / 2;
	if (mtmp->mhp < 1) {
	    if (canseemon(mtmp))
	...
	if (mtmp->data->mlet == S_EEL && !Is_waterlevel(&u.uz)
				      && !(IS_PUDDLE(levl[mtmp->mx][mtmp->my].typ))) {
...
nexttry:	/* eels prefer the water, but if there is no water nearby,
	...
	for(nx = max(1,x-1); nx <= maxx; nx++)
	  for(ny = max(0,y-1); ny <= maxy; ny++) {
		...
		continue;
	    if((wantpool == (is_pool(nx,ny) || (IS_PUDDLE(levl[nx][ny].typ))
		&& !bigmonst(mon->data)) || poolok) && (lavaok || !is_lava(nx,ny)) &&
		/* hack: iron golems and Shai-hulud avoid shallow water */
		((mon->data != &mons[PM_IRON_GOLEM] && !is_longworm(mon->data))
		|| !IS_PUDDLE(levl[nx][ny].typ))) {
		int dispx, dispy;
...
restartcham()
{
	...
	for (mtmp = fmon; mtmp; mtmp = mtmp->nmon) {
		...
		if ((mtmp->data->mlet == S_MIMIC || mtmp->data == &mons[PM_XEROC]) && mtmp->msleeping &&
				cansee(mtmp->mx, mtmp->my)) {
			set_mimic_sym(mtmp);
			newsym(mtmp->mx,mtmp->my);
		}
