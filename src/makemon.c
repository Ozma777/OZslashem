 ...
 */
	switch (ptr->mlet) {
		...
		if(is_mercenary(ptr) || mm == PM_SHOPKEEPER
			...
			case PM_PIRATE:
			  switch(rnd(3)){
			    case 1: mongets(mtmp, SILVER_SABER); break;
			    case 2: mongets(mtmp, SCIMITAR); break;
			    case 3: mongets(mtmp, KNIFE); break;
			    default: break;
			  }
			  break;
			case PM_PIRATE_KING:
			  mongets(mtmp, SILVER_SABER);
			  mongets(mtmp, SPEED_BOOTS);
			  mongets(mtmp, AMULET_OF_REFLECTION);
			  break;
#endif // PIRATE
		 ...
		 if (mm >= PM_ARCHEOLOGIST && mm <= PM_WIZARD && rn2(4)) {
		  switch (mm) {
		   ...
		   case PM_UNDEAD_WANDKEEPER:/*
		     ...
		     if (!rn2(100)) (void) mongets(mtmp, rnd_misc_item_new(mtmp));
		     if (!rn2(100)) (void) mongets(mtmp, rnd_misc_item_new(mtmp));
		     if (!rn2(100)) (void) mongets(mtmp, rnd_misc_item_new(mtmp));doesn't exist in extern.h and unsure how to add these*/
		     ...
		     if (!rn2(5)) (void)mongets(mtmp, WAN_ENLIGHTENMENT);
		     if (!rn2(5)) (void)mongets(mtmp, WAN_CREATE_MONSTER);
		     if (!rn2(5)) (void)mongets(mtmp, WAN_LOCKING);
			 if (!rn2(5)) (void)mongets(mtmp, WAN_OPENING);
			 if (!rn2(5)) (void)mongets(mtmp, WAN_HEALING);
		     if (!rn2(5)) (void)mongets(mtmp, WAN_EXTRA_HEALING);
		     ...
		     if (!rn2(100)) (void)mongets(mtmp, WAN_NOTHING);
		     if (!rn2(50)) (void)mongets(mtmp, WAN_CREATE_HORDE);
		     if (!rn2(50)) (void)mongets(mtmp, WAN_SPEED_MONSTER);
		     if (!rn2(5)) (void)mongets(mtmp, WAN_DIGGING);
		     if (!rn2(15)) (void)mongets(mtmp, WAN_LIGHT);
#ifndef GOLDOBJ
		     mtmp->mgold = (long) d(mtmp->m_lev, 20);
#else
		     mkmonmoney(mtmp, (long) d(mtmp->m_lev, 20));
		   ...
		   case PM_VEELA:
		     ...
		     mkmonmoney(mtmp, (long) d(mtmp->m_lev, 15));
#endif
		   break;/*because tech.c is complaining about: braces around scalar initializer*/
		   case PM_WIZARD:
		     (void)mongets(mtmp, ATHAME);
		     (void) mongets(mtmp, rnd_offensive_item(mtmp));
		     (void) mongets(mtmp, rnd_offensive_item(mtmp));
 ...
 */
	switch(ptr->mlet) {
		...
		if(is_mercenary(ptr)
		    ...
		    (void) mongets(mtmp,SKELETON_KEY);/*next part not in slashTNG OR extended*//*
		} else if (is_educator(mtmp)) {
 		    if (!rn2(5) || is_principal(mtmp))/* They're going to need it :-) *//*
		...
		break;
	    case S_QUANTMECH:/* not messing with the Schroedinger's Cat box code from TNG but maybe I should add potion of radium and stuff */
...
}Let's see if this is supposed to be here. Yes otherwise uncommon and align_shift get broken
but slashem-extended doesn't have it like this and neither does SlashTNG nor Slashem-orig: 
conclusion took out is_educator (two left brackets and no right bracket)*/
...
int mndx;
{
	...
	if (mons[mndx].geno & (G_UNIQ)) return TRUE;*//*went back*/
	if (mvitals[mndx].mvflags & G_GONE) return TRUE;
	if (Inhell)
		return(mons[mndx].maligntyp > A_NEUTRAL);
	else
		return((mons[mndx].geno & G_HELL) != 0);/*
	return FALSE;*//* Amy changed this commenting out if (inhell) to the !=0 I changed back just incase*/
}
...
register struct permonst *ptr;
{
    ...
    return alshift;/*hope that amy's code works because currently my game doesn't like what was here before*/
}
