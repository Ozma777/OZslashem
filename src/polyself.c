...
newman()
{
	...
	if (u.ulevel > 127 || u.ulevel < 1) { /* level went below 0? */
	    ...
	    if (!Race_if(PM_UNGENOMOLD)) goto dead;
	}
	...
	if (u.uhp <= 0 || u.uhpmax <= 0) {
		if (Polymorph_control || Race_if(PM_UNGENOMOLD) ) {
		    if (u.uhp <= 0) u.uhp = 1;
...
boolean forcecontrol;     
{
	...
	if(!Polymorph_control && !forcecontrol && !draconian && !iswere &&
		...
		if ( (rn2(12) > ACURR(A_CON) || !rn2(50)) && !Race_if(PM_UNGENOMOLD)) {
	...
	if (mntmp < LOW_PM) {
		...
		} while((!polyok(&mons[mntmp]) || is_placeholder(&mons[mntmp]) || (notake(&mons[mntmp]) && rn2(4) ) || ((mons[mntmp].mlet == S_BAT) && rn2(2)) || ((mons[mntmp].mlet == S_EYE) && rn2(2) ) || ((mons[mntmp].mmove == 1) && rn2(4) ) || ((mons[mntmp].mmove == 2) && rn2(3) ) || ((mons[mntmp].mmove == 3) && rn2(2) ) || ((mons[mntmp].mmove == 4) && !rn2(3) ) || ( (mons[mntmp].mlevel < 10) && ((mons[mntmp].mlevel + 1) < rnd(u.ulevel)) ) || (!haseyes(&mons[mntmp]) && rn2(2) ) || ( is_nonmoving(&mons[mntmp]) && rn2(5) ) || ( is_eel(&mons[mntmp]) && rn2(5) ) || ( is_nonmoving(&mons[mntmp]) && rn2(20) && (Race_if(PM_UNGENOMOLD) ) ) || ( is_eel(&mons[mntmp]) && rn2(20) && (Race_if(PM_UNGENOMOLD)) ) )
...
#ifdef JEDI
    if ((otmp = uarmh) != 0 && (youmonst.data == &mons[PM_MIND_FLAYER] || youmonst.data == &mons[PM_ILLITHID] ||
...
dobreathe()
{
	...
	if (Race_if(PM_DOPPELGANGER)
		...
		|| (Role_if(PM_AETHERIC_MAGE) && u.umonnum == PM_YELLOW_DRAGON)
		|| (Role_if(PM_METAL_MAGE) && u.umonnum == PM_BLUE_DRAGON)
		|| (Role_if(PM_EARTH_MAGE) && u.umonnum == PM_STONE_DRAGON))
	    energy = 15;
	else
	    energy = 20;/* a bit more expensive --Amy; sounds good to me too*/
	...
	*fish_parts[] = { "fin", "eye", "premaxillary", "pelvic axillary",
	...
	if (part == HAND || part == HANDED) {	/* some special cases */
	    if (mptr->mlet == S_DOG || mptr->mlet == S_FELINE ||
		    mptr->mlet == S_YETI || mptr == &mons[PM_RATLING])
...
#define EN_ADULT_DRAGON 15
	...
	boolean scales = ((uarm && uarm->otyp == RED_DRAGON_SCALES
			  ...
			  (uarm && uarm->otyp == YELLOW_DRAGON_SCALES   
				&& Role_if(PM_AETHERIC_MAGE)) ||
			  (uarm && uarm->otyp == BLUE_DRAGON_SCALES
				&& Role_if(PM_METAL_MAGE))/*
			  (uarm && uarm->otyp == STONE_DRAGON_SCALES 
				&& Role_if(PM_EARTH_MAGE))*/);
	boolean scale_mail = ((uarm && uarm->otyp == RED_DRAGON_SCALE_MAIL
			  ...
			  (uarm && uarm->otyp == BLUE_DRAGON_SCALE_MAIL 
				&& Role_if(PM_METAL_MAGE))/*
			  (uarm && uarm->otyp == STONE_DRAGON_SCALE_MAIL 
				&& Role_if(PM_EARTH_MAGE))*/);
	 ...
	 */
	if (Upolyd && !Race_if(PM_UNGENOMOLD) && (Race_if(PM_DOPPELGANGER) ||
			(Role_if(PM_FLAME_MAGE) && (u.umonnum == PM_RED_DRAGON || 
			...
			(Role_if(PM_EARTH_MAGE) && (u.umonnum == PM_STONE_DRAGON || 
				u.umonnum == PM_BABY_STONE_DRAGON)))) {
	    rehumanize();
	    return 1;	    
	}

	if ((Role_if(PM_ICE_MAGE) || Role_if(PM_FLAME_MAGE) || Role_if(PM_AETHERIC_MAGE) || Role_if(PM_METAL_MAGE) || Role_if(PM_EARTH_MAGE)) && (yn("Transform into your draconic form?") == 'y') && 
		...
		if (u.ulevel > 13 && u.uen > EN_ADULT_DRAGON || 
		    ...
		    draconic.mon = Role_if(PM_AETHERIC_MAGE) ? PM_YELLOW_DRAGON : Role_if(PM_FLAME_MAGE) ?
		...
		/* Otherwise use the baby form */
		    ...
		    draconic.mon = Role_if(PM_AETHERIC_MAGE) ? PM_BABY_YELLOW_DRAGON : Role_if(PM_FLAME_MAGE) ?
	...
	if (Race_if(PM_DOPPELGANGER) || Race_if(PM_UNGENOMOLD)) {
	    ...
	    pline("You can't polymorph at will%s.", 
		    ((Role_if(PM_FLAME_MAGE) || Role_if(PM_ICE_MAGE) || Role_if(PM_AETHERIC_MAGE) || Role_if(PM_METAL_MAGE) || Role_if(PM_EARTH_MAGE) || 
			Race_if(PM_HUMAN_WEREWOLF) || Race_if(PM_DOPPELGANGER) || Race_if(PM_UNGENOMOLD)) ?
		    " yet" : ""));
	    return 0;
	}
