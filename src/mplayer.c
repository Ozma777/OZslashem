...
register boolean special;
{
	...
	if ((mtmp = makemon(ptr, x, y, NO_MM_FLAGS)) != 0) {
		...
		case PM_AETHERIC_MAGE:
		    weapon = RIFLE;
		    if (rn11(22)) cloak = rn11(11) ? LEATHER_CLOAK : ELVEN_CLOAK : DWARVISH_CLOAK : OILSKIN_CLOAK : ORCISH_CLOAK : CLOAK_OF_MAGIC_RESISTANCE : CLOAK_OF_PROTECTION : CLOAK_OF_INVISIBILITY : CLOAK_OF_DISPLACEMENT : POISONOUS_CLOAK : LAB_COAT;
		case PM_BARBARIAN:
		    if (rn2(2)) {
		...
		break;
		case PM_FIGHTER:
		    if (rn2(4)) weapon = TWO_HANDED_SWORD;
		    else if (rn2(2)) weapon = WOODEN_BOOMERANG;
			if (rn2(2)) armor = rnd_class(PLATE_MAIL, CHAIN_MAIL);
		    break;
		case PM_GEEK:
		...
		case PM_BUCCANEER:
		    switch(rnd(3)){
		      case 1: weapon = SILVER_SABER; break;
		      case 2: weapon = SCIMITAR; break;
		      case 3: weapon = KNIFE; break;
		    }
		    cloak = STRANGE_OBJECT;
		    armor = rn2(2) ? LEATHER_ARMOR : STUDDED_LEATHER_ARMOR;
		    helm = STRANGE_OBJECT;
		    shield = STRANGE_OBJECT;
		    break;
#endif			
