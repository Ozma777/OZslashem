...
/*		else return !chwepon(sobj,
	...
	case SCR_GENOCIDE:
		...
		break;
        case SCR_LEET_HAX:
                You("read the scroll of leet hax.");
                pline("A 1337 choir sings uber1337 songs.");
                You("PWN the universe.");
                if (sobj->blessed) do_leethax(0);
                else if(sobj->cursed) do_leethax(1);  // Kill if cursed
                // uncursed, do nothing
                break;
        case SCR_LIGHT:
		if(!Blind) known = TRUE;
	...
	case SCR_FIRE:
		 ...
		 * some damage under all potential cases. (except blessed)
		 */
		cval = bcsign(sobj);
		if(!objects[sobj->otyp].oc_name_known) more_experienced(0,10);
		if (carried(sobj)) useup(sobj);/* this is commented out in the blessed firescroll no hurt user */
		else useupf(sobj, 1L);
		makeknown(SCR_FIRE);
		if(confused) {
		    if(Fire_resistance) {
		...
		if (Underwater)
		    ...
		    burn_away_slime();*/
			pline("A %s of flame erupts %s%s!",
			cval == 1 ? "ring" : "tower",
			cval == 1 ? "around you" : "from ",
			cval == 1 ? "" : the(surface(u.ux,u.uy)));
		}
		explode(u.ux, u.uy, ZT_SPELL(ZT_FIRE), (2*(rn1(3, 3) + 2 * cval) + 1)/3,
							SCROLL_CLASS, EXPL_FIERY);/* the next MIGHT need extern readjust so I skipped
		explode(u.ux, u.uy, 0,(genericptr_t)sobj, rn1(3, 3) + 2 * cval);
		useup(sobj);no blessed scroll of fire hurts user*/
		return(1);
	case SCR_EARTH:
	    /* TODO: handle steeds */
	    if (
...
register boolean on;
{
	char is_lit;	/* value is irrelevant; we use its address
		...
		set_lit, (genericptr_t)(on ? &is_lit : (char *)0));
		} took out because it complained (might be an extended X role or something)*/
...
do_leethax(int status) {
   if (status == 0) { // blessed, kill everything on level but self
      register struct monst *mtmp, *mtmp2;

      unsigned int gonecnt = 0;
      for (mtmp = fmon; mtmp; mtmp = mtmp2) {
          mtmp2 = mtmp->nmon;
          if (DEADMONSTER(mtmp)) continue;
          mongone(mtmp);
      ...
      return;
       ...
       You("don't PWN enough to continue.");
       u.uhp = -1;
       killer_format = KILLED_BY_AN;
       killer = "scroll of leet hax";
       done(DIED);
   }
   else {
       // This doesn't happen
       pline("Everything all assplode!");
   }
}
