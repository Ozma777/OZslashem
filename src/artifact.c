...
long wp_mask;
{
	...
	if (spfx & SPFX_SCLAIR) {
 	    if(on) ESeeking |= wp_mask;
 	    else ESeeking &= ~wp_mask;
 	    if(on) EClairvoyant |= wp_mask;
 	    else EClairvoyant &= ~wp_mask;
 	    vision_recalc(0);
 	}
	if (spfx & SPFX_HALRES) {
...
int dieroll; /* needed for Magicbane and vorpal blades */
{
	...
	*dmgptr += spec_dbon(otmp, mdef, *dmgptr);
	...
	if (attacks(AD_ELEC, otmp)) {
	    if (realizes_damage)
                pline_The("charged weapon hits%s %s%c",
...
nothing_special:
	...
	case IDENTIFY:
	  {
		struct obj *pseudo = mksobj(SCR_IDENTIFY, FALSE, FALSE);
		pseudo->blessed = 1;
		pseudo->cursed = 0;
