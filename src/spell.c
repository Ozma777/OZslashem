 ...
 *  Brd to put beings to sleep (SPE_SLEEP)
...
boolean atme;
{
	...
	if (spellid(spell) == SPE_CHEMISTRY) {
...
if (crball_divin(uwep,spell)) {
	...
	{	register int cnt = 1;
		...
		if (role_skill >= P_SKILLED) cnt += (role_skill - P_BASIC);
		while(cnt--) {
                        mtmp = make_helper((pseudo->otyp == SPE_GAS_SPHERE) ?
                                        PM_GAS_SPHERE : PM_SHOCKING_SPHERE, u.ux, u.uy);
			if (!mtmp) continue;
			mtmp->mtame = 10;
			mtmp->mhpmax = mtmp->mhp = 1;
			mtmp->isspell = mtmp->uexp = TRUE;
        ...
        case SPE_CONFUSING_SPHERE:
        case SPE_SLUMBER_SPHERE:
	{	register int cnt = 1;
		struct monst *mtmp;
		...
		if (role_skill >= P_SKILLED) cnt += (role_skill - P_BASIC);
		while(cnt--) {
                        mtmp = make_helper((pseudo->otyp == SPE_CONFUSING_SPHERE) ?
                                        PM_CONFUSING_SPHERE : PM_SLUMBER_SPHERE, u.ux, u.uy);
			if (!mtmp) continue;
			mtmp->mtame = 10;
			mtmp->mhpmax = mtmp->mhp = 1;
			mtmp->isspell = mtmp->uexp = TRUE;
        ...
        case SPE_VOID_SPHERE:
        case SPE_POISON_SPHERE:
	{	register int cnt = 1;
		struct monst *mtmp;
		...
		if (role_skill >= P_SKILLED) cnt += (role_skill - P_BASIC);
		while(cnt--) {
                        mtmp = make_helper((pseudo->otyp == SPE_VOID_SPHERE) ?
                                        PM_VOID_SPHERE : PM_POISON_SPHERE, u.ux, u.uy);
			if (!mtmp) continue;
			mtmp->mtame = 10;
			mtmp->mhpmax = mtmp->mhp = 1;
			mtmp->isspell = mtmp->uexp = TRUE;
