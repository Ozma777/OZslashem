...
register int amount;
{
	...
	if(uwep->otyp == CRYSKNIFE && !uwep->oartifact && amount < 0) {
	...
	if(uwep->otyp == WOODEN_BOOMERANG && amount >= 0) {
		uwep->otyp = MAGIC_BOOMERANG;
		Your("weapon seems strangely energized.");
		uwep->cursed = 0;
		return(1);
	}
		
	if(uwep->otyp == MAGIC_BOOMERANG && amount < 0) {
		uwep->otyp = WOODEN_BOOMERANG;
		Your("weapon seems plainer now.", body_part(HAND));
		return(1);
	}
