...
/*
STATIC_OVL boolean without_the()
{
 	char *s;
 
 	s = rank();
 
 	if ((s[0] == 'H' || s[0] == 'h') && s[1] == 'e') return(TRUE);
 	if ((s[0] == 'S' || s[0] == 's') && s[1] == 'h' && s[2] == 'e') return(TRUE);
 	return(FALSE);
}*/ /* none of the other stuff existed(ifdef polyself and stuff so I commented this out and didnt code the rest) */
...
*/
	...
	if(Feared)
		Sprintf(nb = eos(nb), bot2_abbrev >= 2 ? " Frd" : " Fear");*/
	if(Hallucination)
