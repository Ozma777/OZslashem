...
#ifdef OVLB
STATIC_DCL const char *FDECL(food_xname, (struct obj *,BOOLEAN_P));/* amy changed all of these so I will too put them back*/
...
STATIC_DCL void FDECL(cprefx, (int));dnethack put it in extern*/
STATIC_DCL int FDECL(intrinsic_possible, (int,struct permonst *));
STATIC_DCL void FDECL(givit, (int,struct permonst *));/*
STATIC_DCL void FDECL(cpostfx, (int));dnethack also put it in extern*/
...
#define CANNIBAL_ALLOWED() (Role_if(PM_CAVEMAN) || Race_if(PM_ORC) || \
			    Race_if(PM_HUMAN_WEREWOLF) || Race_if(PM_OGRE) || Race_if(PM_KOBOLD) || Race_if(PM_VAMPIRE))
...
register struct obj *obj;
{
	...
	if(obj->otyp == CORPSE && (obj->corpsenm == PM_AOA || obj->corpsenm == PM_AOA_DROPLET || obj->corpsenm == PM_NEWT)) return TRUE;already below*/
	if(obj->otyp == CORPSE && (/*obj->corpsenm == PM_AOA || obj->corpsenm == PM_AOA_DROPLETtake out what I dont have*/obj->corpsenm == PM_NEWT || obj->corpsenm == PM_DEMINEWT || obj->corpsenm == PM_MASTER_NEWT)) return TRUE;
...
struct obj *obj;
{
	...
	if(etype == HELLFIRE_FURNACE && !(obj->oartifact)){
		if(objects[(otmp)->otyp].oc_material >= WAX && objects[(otmp)->otyp].oc_material <= BONE
#ifdef MAIL
			&& obj->otyp != SCR_MAIL
#endif
		)
			return TRUE;
	}okay if otmp is going to error out thenI will comment out*/
...
register struct obj *obj;
{
	...
	/* Ghasts, ghouls, and wraiths only eat corpses Also I want trolls to eat old corpses too (oz) */
	if (u.umonnum == PM_GHOUL || u.umonnum == PM_GHAST || u.umonnum == PM_WRAITH || (Race_if(PM_GHAST) && !Upolyd) || (Race_if(PM_WRAITH) && !Upolyd) )
...
#define SUSHI_TIN 10
 	{"sushi",     70},	
#define SZECHUAN_TIN 11
 	{"szechuan",     70},
#define DRIED_TIN 12
	{"dried",        55},
#define FRENCH_FRIED_TIN 13
	{"french fried", 40},
	{"sauteed",      95},
	{"broiled",      80},
	{"smoked",       50},
...
}/* added because function wasn't stopping*/
...
/*STATIC_OVL */void/*(also just void in dnethack)
cprefx(pm) dnethack is different*/
cprefx(pm,bld,nobadeffects)/* dnethack */
/*register */int pm;/* no register in dnethack */
BOOLEAN_P bld, nobadeffects;/* dnethack */
{
...
/*STATIC_OVL */void/* dnethack is different (defined in extern differently)(and no static either)
cpostfx(pm)		/* called after completely consuming a corpse */
cpostfx(pm, tin, nobadeffects, drained)		/* called after completely consuming a corpse (dnethack)*/
register int pm;
BOOLEAN_P tin, nobadeffects, drained;/* dnethack */
{
	register int tmp = 0;
...
violated_vegetarian()
{
    ...
    if (Role_if(PM_MONK) && Race_if(PM_SYLPH)) {
	You_feel("guilty.");
	adjalign(-1);
    }
    return;
...
opentin()		/* called during each move whilst opening a tin */
{
	...
	if(tin.tin->spe != 1) {
		...
		r = DRIED_TIN;		/* lizards don't rot *//* declared before and so double ROTTEN_TIN?_NOPE exists in original*/
		else if (tin.tin->spe == -1 && !tin.tin->blessed && !rn2(7))/* dnethack */
			...
			if(Hallucination && u.clockworkUpgrades&(WOOD_STOVE|MAGIC_FURNACE|HELLFIRE_FURNACE|SCRAP_MAW)){
				...
				goto use_me;
						...
						(tin.tin->corpsenm == PM_NEWT || 
						 tin.tin->corpsenm == PM_DEMINEWT || 
						 tin.tin->corpsenm == PM_MASTER_NEWT))
			)){
	    ...
	    You("consume %s %s.", tintxts[r].txt,
			mons[tin.tin->corpsenm].mname);
			  ...
			  /* SUSHI_TIN might have free fortune cookie */
              if(r==SUSHI_TIN && rn2(2)) {
	    ...
	    if (mvitals[tin.tin->corpsenm].eaten)
#endif
	    tin.tin->dknown = tin.tin->known = TRUE;/* changed to dnethacks version below this
	...
	register struct obj *otmp;
{
	...
	if (mnum != PM_ACID_BLOB && !stoneable && rotted > 5L) {
		...
		return(2);
		   ...
		   youmonst.data == &mons[PM_WRAITH] || 
		   youmonst.data == &mons[PM_TROLL]) {
...
struct obj *otmp;
{
	...
	if (cadaver || otmp->otyp == EGG || otmp->otyp == TIN) {
		...
		if (mnum == PM_SLIME_GIANT)
		    stoneorslime = (!Unchanging && !flaming(youmonst.data) &&
			youmonst.data != &mons[PM_SLIME_GIANT]);
	...
	if (cadaver && !vegetarian(&mons[mnum]) &&
	    !u.uconduct.unvegetarian && Role_if(PM_MONK) && Race_if(PM_SYLPH)) {
...
/* original that I broke by just placing added things anywhere
void
...
#endif /* CONVICT *//*
		&& (!Race_if(PM_SYLPH) || (moves % 2) || (u.uhs < HUNGRY))*//*sylph last long but also add use more food around regen code*
...
/* dnethacks version without seals*/
void
gethungry()	/* as time goes by - called by moveloop() and domove() */
{
	int hungermod = 1;
	...
	if ((carnivorous(youmonst.data) 
		...
		&& !( (Slow_digestion && !Race_if(PM_INCANTIFIER)) ||
			...
			(Race_if(PM_INCANTIFIER) ? u.uen-- : u.uhunger--);		/* ordinary food consumption *//*
	if(uwep && (
		uwep->oartifact == ART_GARNET_ROD || (uwep->oartifact == ART_TENSA_ZANGETSU && !is_undead(youmonst.data)))neither artifact is in my game
	)need this {?*/{
		if(Race_if(PM_INCANTIFIER)) u.uen -= 9;
		else u.uhunger -= 9;
	}
...
register int num;
{
	...
	}/* added so } at end was reconnected with first{*/
	newuhs(FALSE);
}
...
boolean incr;
{
	...
	newhs = (h > (Race_if(PM_INCANTIFIER) ? max(u.uenmax/2,200) : u.uhungermax/2) ) ? SATIATED :/* took the next part out even the added else
	else
