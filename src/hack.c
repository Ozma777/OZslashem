...
#define HERB_GROWTH_LIMIT    8 /* to limit excessive farming */
    ...
    xchar x, y;
{
	...
	You("start chewing %s %s.",
	    (boulder || IS_TREE(lev->typ) || IS_COLUMN(lev->typ)) ?
	    "on a" : "a hole in the",
	    boulder ? "boulder" : IS_TREE(lev->typ) ? "tree" :
	    IS_COLUMN(lev->typ) ? "column" : IS_ROCK(lev->typ) ? "rock" : "door");
	...
	lev->typ = ROOM;
	digtxt = "eat the column.";
	if (column_collapse) {
	    You(digtxt);
	    pline("CRASH! The unsupported ceiling collapses!");/*
	    drop_earth((int)x,(int)y,0,FALSE,FALSE);*/
	    unblock_point(x, y);	/* vision */
	    newsym(x, y);
	    (void) memset((genericptr_t)&digging, 0, sizeof digging);
	    return 1; /* so you don't automatically eat the boulder */
	}
...
int mode;
{
	...
	if (tmpr->typ == IRONBARS) {
	    ...
	    ;	/* do nothing */
	} else if ((tunnels(youmonst.data) && !needspick(youmonst.data)) || (Race_if(PM_RATLING) && !Upolyd)) {
	    /* Eat the rock. */
	...
	if (mode == DO_MOVE) {
	    ...
	    if ((tunnels(youmonst.data) && !needspick(youmonst.data) || (Race_if(PM_RATLING) && !Upolyd)) &&
		!In_sokoban(&u.uz)) {
...
domove()
{
	...
	if(mtmp) {
		...
		/* Good joke, huh? */
		if (mtmp->data == &mons[PM_DISPLACER_BEAST] || mtmp->data->mlet == S_GRUE || mtmp->data == &mons[PM_SHIMMERING_DRAGON] && !rn2(2))
...
struct monst *mon;
{
	...
	if (!(canseemon(mon) || (u.ustuck == mon && u.uswallow && !Blind))
		 || !Role_if(PM_HEALER) && !Role_if(PM_SCIENTIST) && !Role_if(PM_NECROMANCER))
	    return (char *)0;
...
stillinwater:;
	if (!Levitation && !u.ustuck && !Flying) {
		...
		if (verysmall(youmonst.data)) water_damage(invent, FALSE,FALSE);
#ifdef STEED
		if (!u.usteed)
#endif
...
weight_cap()
{
	...
	if (Race_if(PM_RATLING)) carrcap /= 3;
	if (Race_if(PM_DRAKELING)) carrcap /= 2;
