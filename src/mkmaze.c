...
boolean doinit;
{
    if (doinit || (rn2(100) < 20)) {
	xchar n;
	for (n = rn2(6)+3; n; n--) {
	    xchar x = rn1(COLNO-4,3);
	    xchar y = rn1(ROWNO-4,3);
	    struct trap *ttmp = t_at(x,y);
	    if (levl[x][y].typ == LAVAPOOL || (ttmp && ttmp->ttyp == FIRE_TRAP)) {
		(void) create_gas_cloud(x,y, 2+rn2(5), rn1(10,5));
		if (!doinit && flags.soundok)
		    Norep("You hear a loud whooshing sound!");
	    }
	}
    }
}
