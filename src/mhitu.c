...
register struct attack *mattk;
{
	...
	if((compat = could_seduce(mtmp, &youmonst, mattk)) && !mtmp->mcan &&
		...
		case AT_BUTT:
			...
			break;/* Amy's next attacks are better, but too complex right now*/
		case AT_LASH:
...
/*	Now, adjust damages via resistances or specific attacks */
	switch(mattk->adtyp) {
		...
		break;
	    case AD_SLEE:/*is_educator was here but extended doesn't have it*/
		hitmsg(mtmp, mattk);
...
boolean ufound;
{
	...
	switch (mattk->adtyp) {
		...
		case AD_DRST:
		not_affected |= Poison_resistance;
		goto common;
		...
		case AD_PHYS: /* just do some plain physical damage, thanks AmyBSOD for the code I hope your ok with copy/paste because I do NOT know what I am doing*/
...
register struct monst *mon;
{
	...
	if (rn2(35) > ACURR(A_CHA) + ACURR(A_INT)) {
		...
		mon->mspec_used = rnd(100); /* monster is worn out *//* next part will be hard to implement
		if(!uclockwork){ and some clockwinding stuff*/
