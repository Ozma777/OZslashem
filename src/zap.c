...
struct obj *otmp;
{
	...
	case SPE_SLUMBER_SPHERE:	/* (broken wand) */
		/* [wakeup() doesn't rouse victims of temporary sleep,
		    so it's okay to leave `wake' set to TRUE here] */
		reveal_invis = TRUE;
		if (sleep_monst(mtmp, d(1 + otmp->spe, 12), WAND_CLASS))
...
struct obj *obj, *otmp;
{
	...
	case SPE_FORCE_BOLT: {
			...
			if (!flags.mon_moving)
			    seen_it = hero_breaks(obj, obj->ox, 
						  obj->oy, FALSE);
			else
			    seen_it = breaks(obj, obj->ox, obj->oy);
			res = 0;
		}
		/* BUG[?]: shouldn't this depend upon you seeing it happen? or did this get fixed with statue patch?*/
		if (seen_it) {
		makeknown(otmp->otyp);
		}
		break;
...
boolean			youattack, allow_cancel_kill, self_cancel;
{
	...
	if (youdefend) {
		...
		if(u.uen<0 && !Race_if(PM_INCANTIFIER)) u.uen = 0; not messing with this because slashem has an extra } here*/
	} else {
	    mdef->mcan = TRUE;
	...
	case SPE_STONE_TO_FLESH:
		...
		*/
		...
		if (!(e && e->engr_type == ENGRAVE)) {
			...
			if (IS_PUDDLE(levl[u.ux][u.uy].typ))
