...
char *buf;
{
	...
	else if (IS_COLUMN(maploc->typ)) what = "a column";
...
dokick()
{
	...
	if ((is_pool(x, y) || IS_PUDDLE(levl[x][y].typ)) ^ !!u.uinwater) {
	...
	if(!IS_DOOR(maploc->typ)) {
		...
		if(IS_COLUMN(maploc->typ)) {
		    if ((bigmonst(youmonst.data) || martial()) && rn2(acurrstr()) > 18) {
			levl[x][y].typ = ROOM;
			unblock_point(x, y);
			pline("Your devastating kick shatters the column!");
			/* quick 'n' dirty way of making rocks */
			fracture_rock(mksobj_at(BOULDER,x,y,FALSE,FALSE));
			if (column_collapse) {
			    pline("CRASH! The unsupported ceiling collapses!");/*
			    drop_earth(x,y,0,FALSE,TRUE);*/
			}
			exercise(A_DEX, TRUE);
			return(1);
		    } else goto ouch;
