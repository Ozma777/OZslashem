...
static const char *random_mesg[] = {
	...
	"It is dark. You may be eaten by a grue.", /* ZORK */
#ifdef JEDI
...
/* Quotes from or about the Hitchhikers Guide to the Galaxy */
	...
	"FIQ:  One step from divinity.",
		...
		", the Broken Clockmaker", /* 4 even though this is a seal and needs to be put in later for PM_Exile*/
};
...
doengrave()
{
	...
	switch (otmp->oclass) {
		...
		if (zappable(otmp)) {
			...
			if(!objects[otmp->otyp].oc_name_known) {
			    if (flags.verbose)
				pline("This %s is a wand of acid!",
					xname(otmp));
			    doknown = TRUE;
			}
			if (!Blind) {
			    Strcpy(post_engr_text,
				    "Acid flows from the wand.");
			    doblind = TRUE;
			} else
			    Strcpy(post_engr_text, "You hear a hissing sound!");
			break;
