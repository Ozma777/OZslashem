...
struct mkroom *croom; /* NULL == choose random room */
{
	...
	if (sroom->hx < 0 || (!croom && (sroom->rtype != OROOM ||
 	...
 	while ((tried++ < 50) && (i >= 0) && somexy(sroom, &pos)) {
		...
		if (!MON_AT(pos.x, pos.y) && (pmon = mkclass(S_SNAKE,0))) {
 		struct monst *mtmp = makemon(pmon, pos.x,pos.y, NO_MM_FLAGS);
 		mtmp->msleeping = 1;
 		i--;
 	    }
		if (!MON_AT(pos.x, pos.y) && (pmon = mkclass(S_YETI,0))) {
 		struct monst *mtmp = makemon(pmon, pos.x,pos.y, NO_MM_FLAGS);
 		mtmp->msleeping = 1;
 		i--;
 	    }
...
coord *c;
{
	...
	while(try_cnt++ < 100) {
		c->x = somex(croom);
		c->y = somey(croom);
		if (IS_WALL(levl[c->x][c->y].typ) || IS_COLUMN(levl[c->x][c->y].typ))
		    continue;
