...
domonability()
{
	/*if(uclockwork){
		...
		add_menu(tmpwin, NO_GLYPH, &any,
			...
			MENU_UNSELECTED);
		atleastone = TRUE;
	}original dNethack but I think the following after gremlin will work better*/
	...
	else if (u.umonnum == PM_CLOCKWORK_AUTOMATON) return doclockspeed();/* I hope...but maybe stick in something like "in which direction" only which speed? obviously*/
...
int final;	/* 0 => still in progress; 1 => over, survived; 2 => dead */
{
	...
	if (Seeking) you_have("automatic detect unseen");
	...
	if (Polymorph) you_are("polymorphing");/* now I'm really angry at cmd.c error 1479 'NoPolymorph_control' undeclared (first use in this function)
	...
	if (u.ulycn >= LOW_PM && !uclockwork) {
		Strcpy(buf, an(mons[u.ulycn].mname));
		you_are(buf);
	}wtf nopolymorph_control is triggering?*/
	if (Upolyd) {
