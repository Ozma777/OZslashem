...
register struct obj *obj;
{
	...
	if(u.uswallow) {
		...
		if (obj->otyp == CORPSE) {
		    ...
		    could_slime = (obj->corpsenm == PM_GREEN_SLIME || PM_SLIME_GIANT);
...
struct obj *obj;
{
	...
	case CRYSKNIFE:
	    ...
	    if (!obj->oartifact && (!obj->oerodeproof || !rn2(10))) {
		obj->otyp = WORM_TOOTH;
		obj->oerodeproof = 0;
	    }
	    break;
...
donull()
{
	...
	if(uclockwork){
		...
		}/* else if(u.sealsActive&SEAL_EURYNOME && ++u.eurycounts>5) unbind(SEAL_EURYNOME,TRUE); no seals yet*/
