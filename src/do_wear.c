...
Gloves_off()
{
	...
	case GAUNTLETS_OF_DEXTERITY:
	    ...
	    break;
	case GAUNTLETS_OF_REFLECTION:
		if (!oldprop && !(EReflecting & ~TIMEOUT))
		EReflecting = 0;
		break;
	default: impossible(unknown_type, c_gloves, uarmg->otyp);
    }
...
boolean noisy;
{
    ...
    if (which && cantweararm(youmonst.data) && !Race_if(PM_GIANT)  &&
	...
	if (uarmh) {
	    ...
	    err++;
				...
				youmonst.data == &mons[PM_ILLITHID] ||
