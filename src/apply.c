	...
	if(u.dx || u.dy || u.dz){
		...
		if(mm){
			...
			if((u.dx || u.dy) && (mm->data == &mons[PM_CLOCKWORK_SOLDIER] || mm->data == &mons[PM_CLOCKWORK_DWARF] || 
				...
				mm->data == &mons[PM_GNOME_JUGGERNAUT] || mm->data == &mons[PM_ID_JUGGERNAUT])
			){
				mm->mvar1 = -1;
				while(xdir[(int)(++mm->mvar1)] != u.dx || ydir[(int)mm->mvar1] != u.dy);
			}/*mvar1 doesnt yet exist, yes it does, firework_cart will NOT work even with attk changed*/
		}
		useup(obj);
		*optr = 0;
	} else {
...
doapply()
{
	...
	case ICE_BOX:
	case SACK:
	case UGLY_BACKPACK: /*	I like having the backpack having to be dropped first...realistic for a camping backpack*/
	case BAG_OF_HOLDING:
	case OILSKIN_SACK:/*
...
/*#if 0 my slashem is a bit harder so abuse all you want but it will not help you very much (add elemental orbs as well as scrolls like FIRE)*
	case ORB_OF_MAINTENANCE:
	    if(obj->spe > 0) {
		struct obj *otmp;
		consume_obj_charge(obj, TRUE);
		otmp = getobj(all_count, "repair", getobj_filter_repair);
		...
		if (obj->cursed) {
			...
			int i = 0;
			if (otmp->oeroded && otmp->oeroded2) i = rn2(2);
			else if (otmp->oeroded2) i = 1;
			if (i) otmp->oeroded2--; else otmp->oeroded--;
			Your("%s %s less %s.",
			     xname(otmp), otense(otmp, Blind ? "feel" : "look"),
			     is_metallic(otmp) ? ((i==0) ? "rusty" : "corroded") : "damaged");
		    }
		} else pline(nothing_happens);
	    } else pline("This orb is burnt out.");
	    break;OBJECT doesn't exist yet and also my code is broken enough already*/
	...
	case ORB_OF_CHARGING:
		...
		break;/* Brass code replaces this
	...
	case ORB_OF_DESTRUCTION:
		...
		check_unpaid(obj);
		break;
/*#endif I want these and I'll add a few later too like Brass' orb of repair*/
	case MAGIC_MARKER:
		res = dowrite(obj);
		break;
	case TIN_OPENER:
	...
	case RELAY:
		if (obj->oartifact == ART_BURNED_MOTH_RELAY) {	
			...
			makeknown(RELAY);
			if (Role_if(PM_GEEK) || Role_if(PM_SCIENTIST)) {
				You("feel rememberanced of %s.",Hallucination ? "when the net was flat" : "the old times");
				break;
			}
		}	/* fall through */	
	case DIODE:
