 ...
 * guitar hopeful  	May calm nymph if player has enough dexterity.
...
boolean domsg;
{
	...
	&& (distu(mtmp->mx, mtmp->my) <= 25)) {
	    ...
	    if ((song_instr->otyp == WOODEN_HARP || song_instr->otyp == GUITAR || song_instr->otyp == MAGIC_HARP)
...
int know_spell;
{
	...
	if (uarms && (instr->otyp == WOODEN_HARP || instr->otyp == GUITAR || instr->otyp == LEATHER_DRUM)) 
		chance /= 2;
