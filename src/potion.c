...
#endif
			    Underwater || IS_PUDDLE(levl[u.ux][u.uy].typ) || IS_POOL(levl[u.ux][u.uy].typ)))
	    *qp++ = ALLOW_THISPLACE;
	Strcpy(qp, beverages);
	...
	register struct obj	*otmp;
{
	...
	case POT_GAIN_ENERGY:			/* M. Stephenson */
		{       register int num , num2;
			...
			if(u.uen <= 0 && !Race_if(PM_INCANTIFIER)) u.uen = 0;/* here? yes?*/
			flags.botl = 1;
			exercise(A_WIS, TRUE);
		}
		break;
...
dodip()
{
	...
	if (potion->otyp == POT_FRUIT_JUICE && obj->otyp == CORPSE) {
		...
		case PM_CYAN_MOLD:
		case PM_DISGUSTING_MOLD:
		case PM_BLACK_MOLD:
		case PM_BLUE_JELLY:
		case PM_SCARLET_JELLY:
...
long timeout;
{
	...
	break;
	case PM_CYAN_MOLD:
	new_otyp = POT_GAIN_ENERGY;
	break;
	case PM_DISGUSTING_MOLD:
	new_otyp = POT_SICKNESS;
	break;
	case PM_BLACK_MOLD:
	new_otyp = POT_CLAIRVOYANCE;
	break;
	case PM_BLUE_JELLY:
	new_otyp = POT_EXTRA_HEALING;
	break;
	case PM_SCARLET_JELLY:
	new_otyp = POT_OIL;
	break;
	...
	break;
	case PM_SHRIEKER:
	new_otyp = POT_PARALYSIS;
	break;
	case PM_SPOTTED_JELLY:
	new_otyp = POT_OBJECT_DETECTION;
	break;
	case PM_CLEAR_JELLY:
	new_otyp = POT_WATER;
	break;
