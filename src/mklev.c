...
STATIC_DCL void FDECL(mkcolumn,(struct mkroom *));
...
boolean lava;
{
	...
	else if (levl[cx][cy].typ == CORR) chance = 33;
	else if (levl[cx][cy].typ == ROOM) chance = 66;
...
makelevel()
{
	...
	/* [Tom] totally reorganized this into categories... used
	   ...
	   !(mvitals[PM_LEPRECHAUN].mvflags & G_GONE)) mkroom(LEPREHALL);
        ...
        else if (u_depth > 9 && !rn2(6)) mkroom(GARDEN);
        else if (u_depth > 5 && !rn2(6) &&
	    ...
	    else if(depth(&u.uz) > 13 && !rn2(7)) mkroom(SWAMP);
#ifdef DOUGLAS_ADAMS
...
skip0:
	...
	for(croom = rooms; croom->hx > 0; croom++) {
...
#endif
		/* the deeper you go, the greater the need for structural supports */
		if(!rn2(30 - depth(&u.uz))) mkcolumn(croom);
		...
		/*attempt at trees and stuff*/
		if(!rn2(2)) mktree(croom);
		if(!rn2(4)) mktree(croom);
		if(!rn2(8)) mktree(croom);
		if(!rn2(12)) mktree(croom);
		if(!rn2(2)) mkice(croom);
		if(!rn2(4)) mkice(croom);
		if(!rn2(8)) mkice(croom);
		if(!rn2(12)) mkice(croom);
		if(!rn2(8)) mkironbars(croom);
		if(!rn2(16)) mkironbars(croom);
		if(!rn2(8)) mkcloud(croom);
		if(!rn2(16)) mkcloud(croom);
		if(!rn2(16)) mkpuddles(croom);
#ifdef SINKS
		if(!rn2(30)) {
		    mksink(croom);
...
register struct mkroom *croom;
{
	...
	if ((croom->hx - croom->lx < 4)
	  || (croom->hy - croom->ly < 4)) {
	    /* option 1: place one in the center of the room */
	    m.x = croom->lx + ((croom->hx - croom->lx) / 2);
	    m.y = croom->ly + ((croom->hy - croom->ly) / 2);
	    if (!(occupied(m.x, m.y) || bydoor(m.x, m.y))) {
		levl[m.x][m.y].typ = COLUMN;
	   	 block_point(m.x, m.y);
	    }
	} else {
	    /* option 2: place four near each corner */
	    int i,j;
	    for (i = 0; i<2; i++) for (j = 0; j<2; j++) {
	        m.x = i ? (croom->lx + 1) : (croom->hx - 1);
		m.y = j ? (croom->ly + 1) : (croom->hy - 1);
	...
	do {
	   ...
	   if (puddles > 4 && depth(&u.uz) > 4) {
		(void)makemon(levl[m.x][m.y].typ == POOL ? mkclass(S_EEL,0) :
				&mons[PM_TROUT],m.x,m.y,NO_MM_FLAGS);
		puddles -= 2; /* puddles created should always exceed piranhas note replaced piranhas with trout */
	   }
	   tryct = 0;
	   do {
	       m.x += sgn(rn2(3)-1);
