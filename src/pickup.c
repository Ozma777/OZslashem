...
lootcont:
	...
	for (cobj = level.objects[cc.x][cc.y]; cobj; cobj = nobj) {
		...
		if (cobj->otyp == TRICK_BAG && cobj->spe>0) {
		    int tmp;
		    You("carefully open the bag...");
    ...
    int depthin;
{
    ...
    if ((obj->otyp == WAN_CANCELLATION || obj->otyp == BAG_OF_TRICKS && TRICK_BAG) &&
	    obj->spe <= 0)
	return FALSE;
	...
	if (obj->otyp == PORTABLE_HOLE) {
	  ...
	  for (pass = 1; pass <= 2; pass++) {
		...
		if(u.uswallow) {
		  ...
		  (void) mpickobj(u.ustuck,otmp,FALSE);/* no idea if this is right but I got to put SOMETHING here*/
...
int held;
{
	...
	if ((Is_mbag(obj) && obj->cursed && !rn2(13)) || 
 		 (obj->otyp == BAG_OF_DIGESTION && !rn2(obj->blessed ? 20 : (obj->cursed ? 2 : 10)))) {
	...
	/* Sometimes toss objects if a cursed magic bag. original code when I commented out digestion code*/
