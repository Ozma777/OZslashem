...
dig()
{
	...
	if (Race_if(PM_DWARF))
	    bonus *= 2;
	if (IS_COLUMN(lev->typ))
	    digging.effort *= 2; /* easy to collapse */
#ifdef LIGHTSABERS
	...
	if (digging.effort > 100) {
		...
		if ((obj = sobj_at(STATUE, dpx, dpy)) != 0) {
			...
			digtxt = "The boulder falls apart.";
				...
				IS_TREE(lev->typ) || IS_COLUMN(lev->typ)) {
			...
			if (IS_COLUMN(lev->typ)) {
			    lev->typ = ROOM;
			    digtxt = "The column crumbles.";
			    /* quick 'n' dirty way of making rocks */
			    fracture_rock(mksobj_at(BOULDER,dpx,dpy,FALSE,FALSE));
			    if (column_collapse) {
				pline(digtxt);
				pline("CRASH! The unsupported ceiling collapses!");/*
				drop_earth(dpx,dpy,0,FALSE,TRUE);temp disable*/
				digtxt = (char*)0;
			    }
...
cleanup:
		...
		static const char *const d_target[7] = {
			"", "rock", "statue", "boulder", "door", "column", "tree"
 		};
...
boolean pit_only;
{
	...
	} else if (is_pool(u.ux, u.uy) || is_lava(u.ux, u.uy) || IS_PUDDLE(lev->typ)) {
	...
	if (u.uswallow && attack(u.ustuck)) {
		...
		if (dig_target == DIGTYP_UNDIGGABLE) {
			...
			if (trap && trap->ttyp == WEB) {
			    ...
			    You("need a pick to dig %s.", IS_COLUMN(lev->typ) ?
					"into a column" : "rock");
			...
			static const char * const d_action[7][2] = {
				...
				{"chipping the column","melting through the column"},/*probably won't work for lightsabers*/
    ...
    boolean zap;
{
	...
	if (in_town(x, y) &&
		...
		IS_COLUMN(lev->typ) || IS_FOUNTAIN(lev->typ) || IS_TREE(lev->typ))) {
	    if (!mtmp) {
...
register struct monst *mtmp;
{
	...
	if (closed_door(mtmp->mx, mtmp->my)) {
	    ...
	    return FALSE;
			...
			&& !IS_COLUMN(here->typ)) /* no dig */
	    return FALSE;

	/* Only rock, trees, columns and walls fall through to this point. */
	...
	} else if (IS_COLUMN(here->typ)) {
	    here->typ = ROOM;
	    if (column_collapse) {
		if (cansee(mtmp->mx, mtmp->my))
		    pline("CRASH! The ceiling above %s collapses!", mon_nam(mtmp));
		else
		    You_hear("%s!", Hallucination ? "the sky falling"
						       : "the ceiling collapse nearby");/*
		drop_earth(mtmp->mx,mtmp->my,0,FALSE,FALSE);temp disable*/
	    }
...
/*        struct obj *otmp;*/
	...
	while (--digdepth >= 0) {
		...
		if (IS_WALL(room->typ)) {
		    ...
		    break;
		} else if (IS_COLUMN(room->typ)) {
		    if (!(room->wall_info & W_NONDIGGABLE)) {
			room->typ = ROOM;
			digdepth = 0;
			goto crash_ceiling;
		    } else if (!Blind)
			pline_The("column glows then fades.");
		    break;
...
crash_ceiling:
		    if (column_collapse) {
			if (cansee(zx, zy))
			    pline("CRASH! The ceiling above a disintegrated column collapses!");
			else You_hear("the ceiling collapse nearby!");/*
			drop_earth(zx,zy,0,FALSE,TRUE);temp disable*/
		    }
...
escape_tomb()
{
	...
	if ((Teleportation || can_teleport(youmonst.data)) &&
		...
		if(amorphous(youmonst.data) || Passes_walls ||
		   noncorporeal(youmonst.data) || unsolid(youmonst.data) || (Race_if(PM_RATLING) && !Upolyd) ||
		   (tunnels(youmonst.data) && !needspick(youmonst.data))) {

		    You("%s up through the %s.",
			(tunnels(youmonst.data) && !needspick(youmonst.data)) ?
			 "try to tunnel" : (Race_if(PM_RATLING) && !Upolyd) ? "try to tunnel" : (amorphous(youmonst.data)) ?
			 "ooze" : "phase", surface(u.ux, u.uy));

		    if(tunnels(youmonst.data) && !needspick(youmonst.data))
			good = dighole(TRUE);
		    else if (Race_if(PM_RATLING) && !Upolyd)
			good = dighole(TRUE);
		    else good = TRUE;
		    if(good) unearth_you();
		}
