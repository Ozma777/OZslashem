...
const struct innate {
	...
	aet_abil[] = { { 1, &(HPoison_resistance), "", "" },
                     {   1, &(EAcid_resistance), "", "" },
                     {  23, &(HDrain_resistance), "undrainable", "drainable" },
		     {   0, 0, 0, 0 } },
	...
	exp_abil[] = { {	8, &(HSearching), "perceptive", "" },
		     {	15, &(HPoison_resistance), "hardy", "" },
			 {  17, &(HFast), "quick", "slow" },
		     {	 0, 0, 0, 0 } },
			 
	ear_abil[] = { { 1, &(EStone_resistance), "", "" },
                     {   1, &(HSearching), "", "" },
                     {  23, &(HDrain_resistance), "undrainable", "drainable" },
		     {   0, 0, 0, 0 } },

	fig_abil[] = { {	 1, &(HSleep_resistance), "", "" },
		     {   7, &(HFast), "quick", "slow" },
		     {  15, &(HStealth), "stealthy", "" },
		     {	 0, 0, 0, 0 } },
	...
	gan_abil[] = { {	11, &(HSleep_resistance), "awake", "tired" },
		     {	 13, &(HPoison_resistance), "hardy", "vulnerable" },
		     {	 0, 0, 0, 0 } },
	...
	ins_abil[] = { {	 1, &(HSleep_resistance), "", "" },
		     {   9, &(HFast), "quick", "slow" },
		     {  13, &(HStealth), "stealthy", "" },
		     {	 0, 0, 0, 0 } },
	...
	lib_abil[] = { {	8, &(HSearching), "perceptive", "" },
		     {	 0, 0, 0, 0 } },
	...
	sci_abil[] = { {	 1, &(EAcid_resistance), "", "" },
		     {	 0, 0, 0, 0 } },
	
	tou_abil[] = { {	8, &(HSearching), "perceptive", "" },
		     {	15, &(HPoison_resistance), "hardy", "" },
		     {	 0, 0, 0, 0 } },
...
/*			{  10, &(HFire_resistance), "cool", "warmer" },*/
	...
	gia_abil[] = { {	7, &(HSleep_resistance), "awake", "tired" },
		     {	 0, 0, 0, 0 } },

	gha_abil[] = { {  1, &(HSick_resistance), "", "" },
		     {   1, &(HCold_resistance), "", "" },
		     {   1, &(HSleep_resistance), "", "" },
	...
	ill_abil[] = { {  1, &(HEnergy_regeneration), "", "" },
		     {   13, &(HFast), "quick", "slow" },
		     {   0, 0, 0, 0 } },

	inc_abil[] = { {	1, &(EAntimagic), "", "" },
		     {	 0, 0, 0, 0 } },
	...
	ogr_abil[] = { {	1, &(HPoison_resistance), "", "" },
		     {	 0, 0, 0, 0 } },
			 
	rat_abil[] = { {	1, &(HPoison_resistance), "", "" },
				{  1, &(HSick_resistance), "", "" },
				{  1, &(HFast), "", "" },
				{  1, &(HSlow_digestion), "", "" },
		     {	 0, 0, 0, 0 } },
	...
	syl_abil[] = { {  1, &(HStealth), "", "" },
				{5,  &(HFast), "quick", "slow"},
				{7,  &(HInfravision), "perceptive", "half blind"},
				{11, &(HSee_invisible), "your vision sharpen", "your vision blurring"},/*
    {7,  &(HDisplaced), "elusive", "exposed"},might be cool to add and also prot_shapechangers*/
				{17, &(HDetect_monsters), "perceptive", "dull"},
		     {   0, 0, 0, 0 } },
	...
	vee_abil[] = { {	7, &(HSleep_resistance), "awake", "tired" },
			 {11, &(HSee_invisible), "your vision sharpen", "your vision blurring"},
		     {	 0, 0, 0, 0 } },
	
	yak_abil[] = { {	1, &(HPoison_resistance), "", "" },
				{   9,  &(HFast), "quick", "slow"},
				{  15, &(HSee_invisible), "your vision sharpen", "your vision blurring" },
				{	 0, 0, 0, 0 } },
			 
	yuk_abil[] = { {	1, &(HCold_resistance), "", "" },
		     {  11, &(HSee_invisible), "your vision sharpen", "your vision blurring" },
			 {	 0, 0, 0, 0 } },
...
boolean	inc_or_dec;
{
	...
	if(uclockwork) return; /* Clockwork Automata can't exercise abilities */
...
/* TNG REROLL changes void to static void*/
static void
init_attr_(np)
	register int	np;
{
...
int oldlevel, newlevel;
{
	...
	case PM_AETHERIC_MAGE:	abil = aet_abil;	break;
...
#endif	/* CONVICT */
	case PM_EXPLORATOR:	abil = exp_abil;	break;
	case PM_EARTH_MAGE:	abil = ear_abil;	break;
	case PM_FIGHTER:	abil = fig_abil;	break;
	case PM_FLAME_MAGE:	abil = fla_abil;	break;
	case PM_GEEK:	abil = gee_abil;	break;
	case PM_GANGSTER:           abil = gan_abil;	break;
	case PM_HEALER:         abil = hea_abil;	break;
	case PM_INSTRUMENTALIST:	abil = ins_abil;	break;
	...
	case PM_METAL_MAGE:	abil = met_abil;	break;
	...
	case PM_GHAST:		rabil = gha_abil;	break;
	case PM_HOBBIT:		rabil = hob_abil;	break;
	case PM_OGRE: 		rabil = ogr_abil;	break;
	...
	case PM_RATLING:		rabil = rat_abil;	break;
	...
	case PM_WRAITH:            rabil = wra_abil;	break;
	case PM_YAKUZA:		rabil = yak_abil;	break;
	case PM_YUKI_ONNA:		rabil = yuk_abil;	break;
