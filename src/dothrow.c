...
autoquiver()
{
	...
	for (otmp = invent; otmp; otmp = otmp->nobj) {
		...
		/* Ordinary weapon */ /* boulder throwing won't quiver
		} else if (otmp->oclass == ROCK_CLASS && Race_if(PM_GIANT)(otmp)) {*/
...
/*            if (!fire_weapon) setuwep(obj);                
		...
		if (!Is_airlevel(&u.uz) && !Is_waterlevel(&u.uz) && !Underwater && !ART_BLUNDERBUSS
	...
	} else if(objects[obj->otyp].oc_skill == -P_BOOMERANG && !Underwater) {
	...
	if (u.uswallow) {
		...
		if (obj->otyp == MAGIC_BOOMERANG) {
			pline("%s suddenly reappears in your %s!",
			 Blind ? Something : The(xname(obj)), body_part(HAND));
				obj = addinv(obj);
				(void) encumber_msg();
		if (wep_mask && !(obj->owornmask & wep_mask)) {
		setworn(obj, wep_mask);
...
int thrown;
{
	...
	if (obj->oclass == WEAPON_CLASS || is_weptool(obj) ||
		...
		if (objects[obj->otyp].oc_skill == -P_BOOMERANG)                /* arbitrary */
		    tmp += 4;
		...
		/* blunderbuss attempt */
		if (ammo_and_launcher(obj, launcher) &&
                        (launcher->oartifact == ART_BLUNDERBUSS)) {
		    if (cansee(bhitpos.x,bhitpos.y)) 
			pline("%s explodes in a magical blast!", Doname2(obj));
		    else You_hear("an explosion");
		    explode(bhitpos.x, bhitpos.y, ZT_SPELL(ZT_MAGIC_MISSILE),
			    d(2,6), WEAPON_CLASS, EXPL_MAGICAL);
		}
...
register struct obj *obj;
{
	...
	else 	obfree(obj, (struct obj *)0); /* definitely frees obj, nerf by Amy (thanks Amy)*/
	ret = 1;
