...
int prop;
{
	...
	for(y = y1; y <= y2; y++)
	    for(x = x1; x <= x2; x++)
		if(IS_STWALL(levl[x][y].typ) || IS_COLUMN(levl[x][y].typ))
...
register int humidity;
{
	...
	if (humidity & DRY) {
	    typ = levl[x][y].typ;
	    if (typ == ROOM || typ == AIR || typ == CLOUD
			|| typ == ICE || typ == CORR || typ == PUDDLE)
		return TRUE;
	}
	if (humidity & WET) {
	    if (is_pool(x,y) || is_lava(x,y)/* || levl[x][y].typ == PUDDLE*//*someothertimetry (before lava)->*//* || is_puddle(x,y)*/)
		return TRUE;
	}/*tempdisable puddle the towns with pools now have problems with EVERY SPELLBOOK SCROLL AND METAL having water damage*/
	if (humidity & MOLTEN) {
	    if (is_lava(x,y))
		return TRUE;
	}
