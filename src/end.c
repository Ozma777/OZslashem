...
static NEARDATA const char *deaths[] = {		/* the array of death */
	...
	"exploded after being overwound", "genocided", "disintegrated", "panic", "trickery",
...
static NEARDATA const char *ends[] = {		/* "when you..." */
	...
	"were overwound and exploded", "were genocided", "were disintegrated", "panicked", "were tricked",
	"quit", "escaped", "ascended"
};
...
int how;
{
	...
	u.uhplast = u.uhp;/* keeping this just in case
	if (u.uhunger < 500) {
	    u.uhunger = 500;
	    newuhs(FALSE);
	}*/
...
int how;
{
	...
	if (Race_if(PM_TROLL) && how < GENOCIDED && u.ulevel > 2 && rn2(4) ) {
		pline("But wait...");
	    losexp("failed troll revival", FALSE);
...
boolean identified, all_containers;
{
	...
	for (box = list; box; box = box->nobj) {
		...
		if (box->otyp == BAG_OF_TRICKS && TRICK_BAG) {
