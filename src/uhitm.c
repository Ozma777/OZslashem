...
static void attract_vortices(void)
{
	...
	n = rn2(7) - Luck;
	while (n-- > 0) {
		makemon(mkclass(S_VORTEX,0),u.ux,u.uy, NO_MM_FLAGS);
	}
}
...
int thrown;
{
	...
	if(!thrown && no_obj) {      /* attack with bare hands */
		...
		if (/* if you strike with a bow... */
			...
			tmp = rnd(2);
		    if (!thrown && obj == uwep && objects[obj->otyp].oc_skill == P_BOOMERANG &&
				rnl(4) == 4-1) {
...
register struct attack *mattk;
{
 ...
 physical:
		...
		break;
		case AD_DEPR:
		pline("You feel depressed...");/*probably most of this will NOT work
		if (!rn2(2)) {
		...
		} so I'll completely disable ALL of it temporarily*/
		break;
	    case AD_TLPT:
		if (tmp <= 0) tmp = 1;
		...
		if (!negated && !rn2(3) && !resists_drli(mdef)) {
			int xtmp = d(2,6);
			if (mdef->mhp < xtmp) xtmp = mdef->mhp;/* put the next in after commenting this out/
