...
/*add check for improving*/
		...
		|| (!strcmp(word, "charge") && !is_chargeable(otmp)) /*put this here just like dNethack had it */
...
char *buf;
{
	...
	if (IS_DOOR(ltyp)) {
	    ...
	    dfeature = "pool of shallow water"; changing to the next one to see if it will "draw" puddles on quest maps*/
	else if (IS_PUDDLE(ltyp))
	    cmap = S_PUDDLE;
#ifdef SINKS
